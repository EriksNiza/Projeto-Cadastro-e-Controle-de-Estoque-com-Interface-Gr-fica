...
class Cliente:
    ...
    def resumo(self):
        return f"CPF: {self.cpf} | Nome: {self.nome} | Nascimento: {self.data_nasc} | Telefone: {self.telefone} | Endereço: {self.endereco}"
...
class Produtos:
    def __init__(self, nome, preco, qtd_estoque, min_estoque):
        ...
        if int(min_estoque) <= 0:
            raise ValueError("A quantidade mínima deve ser maior que zero.")
        self.nome = nome
        self.preco = float(preco)
        self.qtd_estoque = int(qtd_estoque)
        self.min_estoque = int(min_estoque)
...
class Vendas:
    def __init__(self, produto, qtd, cliente, forma_pagamento=None, data=None):
        ...
        if int(qtd) > produto.qtd_estoque:
            raise ValueError("Estoque insuficiente.")
        self.data = data or datetime.now().strftime("%d/%m/%Y %H:%M")
        self.produto = produto.nome
        self.qtd = int(qtd)
        self.valorTotal = self.qtd * produto.preco
        self.forma_pagamento = forma_pagamento or "Dinheiro"
        self.cliente = cliente

    def resumo(self):
        return f"{self.data} | Produto: {self.produto} | Qtd: {self.qtd} | Total: R${self.valorTotal:.2f} | Pagamento: {self.forma_pagamento} | Cliente: {self.cliente}"

    def aplicar_desconto(self, percentual):
        if percentual <= 0 or percentual >= 100:
            raise ValueError("Percentual inválido.")
        self.valorTotal -= self.valorTotal * (percentual / 100)
